...
class MessageBuilderService {
  ...
  List<ChatMessage> collapseVersions(
    ...
    // Check if any message has parentId — if none, use flat fallback
    final hasTree = items.any((m) => m.parentId != null);
    if (!hasTree) {
      return _collapseVersionsFlat(items, versionSelections);
    }
  ...
  List<ChatMessage> _collapseVersionsFlat(
    List<ChatMessage> items,
    Map<String, int> versionSelections,
  ) {
  ...
  /// and parentId chains.
  List<ChatMessage> _collapseVersionsTree(
    List<ChatMessage> items,
    Map<String, int> versionSelections,
  ) {
    ...
    // Find root groups (parentId == null for all members)
    ...
    // BFS/DFS from roots, following selected versions
    final out = <ChatMessage>[];
