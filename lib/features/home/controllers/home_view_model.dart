...
class HomeViewModel extends ChangeNotifier {
  ...
  Future<void> _maybeUpdateProactiveCareFor(String conversationId) async {
    ...
    // history 统一用 buildHistory（与后台一致：collapseMessageVersions +
    // truncateIndex + 角色过滤，版本选择取 conversation.versionSelections）。
    ...
    try {
      // 决策纯逻辑统一走 decideNextCareTime（内部含 personaPrompt 占位符替换、
      // memoriesBlock 构建、buildDecisionApiMessages、sendMessageStream、
      // parseDecision 及 try/catch；空 history 返回 null）。
