...
> - 最后更新：2026-07-11（Phase 2：MSG-01～04 已完成）
> - 当前结论：Message Graph 原生写路径已具备 edit user、regenerate assistant、stable-ID selection、revision/slot delete 与 conversation fork 单事务命令。v5 新增最小权威 text part，使 command 不产生无 payload revision；复杂 reasoning/tool/provider parts 仍按计划留给 GEN-05。删除只触及目标因果子树和受影响 branch，完全不调用旧 order compact
...
| Phase 2：Message Graph | 4 / 7 | `进行中` | 原生 graph schema/projector/commands 已完成；下一步实现 Hive/SQLite v1 deterministic legacy adapter |
...
Phase 2 当前 4/7。下一步执行 MSG-05：实现 Hive 公开主源与 SQLite v1 开发机源到 graph/parts 的确定性 adapter，selection ordinal/version 双解释、legacy causality、truncate boundary、orphans/rejects 均持久化 issue，不猜测历史。D4 renderer/RSS 超标不在 Message Graph 阶段扩 scope。
