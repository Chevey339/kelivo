...
| TL-04 | Following/user anchored/jump 状态机 | PD-04/05、TL-03 | `进行中`（因 §10.1 TL-R4 尚待收口） | 用户阅读历史时 stream 不强制跳动；coordinator/scroll/widget/lazy 聚焦回归通过 | 本里程碑提交（2026-07-11） | `TimelineViewportMode` 冻结 followingTail/userAnchored/programmaticJump/loading 四态；滚轮/触摸离底立即退出尾随，分页 loading 恢复原意图，stream 只更新未读/生成态而不移动 anchored viewport。新发送持久化后以 stable user slot programmatic jump 并建立本轮会话专属 anchor；TL-R3 已移除 streaming 尺寸变化后的 post-frame spacer 收缩，生成锚定期改为持有 jump 当帧算出的恒定 spacer，使 content extent 不再逐 chunk `+X/-X` 振荡。锚定模式与 bottom-pin 互斥，程序滚动通知不改变视口意图，内容底部判定明确扣除 spacer；仅显式跳到最新或真实到达内容底部才恢复 following。终态一次性回收与锚点补偿归 TL-R4。coordinator 从 loaded streaming slot 初始化 generation 状态，覆盖首 chunk 前及重开活跃会话场景；新增带生成指示、live-region/button semantics 与中英本地化的“跳到最新”胶囊 |
