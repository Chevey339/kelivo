...
| Phase 3：Generation State Machine | 4 / 7 | `进行中` | GEN-01～04 已完成 begin、三链解耦与统一 terminal 事务；下一步切换 ordered parts 唯一正文权威 |
...
Phase 3 已完成 4/7。下一步按方案进入 `GEN-05`：把 text/reasoning/tool/provider artifacts 完整映射到 ordered parts，在同一 checkpoint/final transaction 中持久化，并完成 `message_part_rows` 唯一正文权威切换；任何绕过 repository 直接改写 `message_rows` 正文的生产路径必须退出。D4 renderer/RSS 超标留在 Phase 4。
