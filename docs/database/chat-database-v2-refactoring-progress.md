...
> - 当前结论：Phase 0/1/3 保持关闭。MSG-R3 与 MSG-R1 已完成：版本切换器使用 DB 权威计数；中部 user 编辑通过单事务 graft 创建 sibling、重挂 on-path child、保持 active branch 与全部后续 slot，旧 revision 仍保留。graph/projector/repository 32/32 与 analyze 通过。重生成设置分流仍进入 MSG-R2；无限跳动与发送弹距进入 TL-R14/R15。§10.3 全部闭环并经用户真机四症状矩阵复测前，Phase 4 与 Phase 2 语义项保持进行中，Phase 5 暂不启动
...
| Phase 2：Message Graph | 6 / 7 | `进行中` | graph timeline 与 stable-ID commands 已接管业务；但 PD-01 于 2026-07-12 修订为默认 graft 语义，命令层需按 §10.3 MSG-R1/R2 返工（MSG-04 重开），schema 不变 |
...
| Phase 4：Timeline 与 Renderer | 6 / 8 | `进行中` | TL-R1～R6 已获第一轮真机确认；§10.2 TL-R7～R13 实现与自动化闭环；§10.3 MSG-R3 已完成，TL-R14/R15 尚待执行，闭环并经真机四症状矩阵前不关闭 |
...
| MSG-04 | Edit/regenerate/select/delete/fork commands | DB2-05、MSG-03 | `进行中`（MSG-R1 已完成，MSG-R2 待完成） | 全部使用稳定 revision/branch ID 且单事务；默认变更走 graft（保留后代），仅删除尾随走 fork；删除不得复用 `_rewriteMessageOrder` 或全会话 compact，D3 删除更新量不得随会话总消息数线性增长 | 本里程碑提交（2026-07-11） | v5 增加 composite FK/ordinal/kind 受约束的最小权威 `message_part_rows`。MSG-R1 新增 graft command：原子创建同 slot revision+text part、把 active path 直接 child re-parent（目标为 leaf 时只移动当前 branch leaf）、映射同 slot context boundary、CAS 推进 state revision且 branch ID 不变；稳定 graph edit 与 `appendMessageVersion(user)` 均已接入，中部编辑后全部后续 slot 保留、旧 revision 未删除、无新增 branch。重生成设置分流仍留 MSG-R2。既有 delete/fork 语义与 OPS-06 债务不变 |
...
实现收敛（2026-07-12，MSG-R1）：新增 `graftRevision` 单事务 command，校验目标在 active path/角色/state revision 后创建同 slot sibling 与 text part；中部目标把 active path 的唯一直接 child 从旧 revision 改挂到新 revision，leaf 目标只更新当前 branch leaf，context boundary 若指向被替换 revision 则映射到新 revision，最后以 CAS 单次推进 state revision。`editMessageGraphUser` 与用户编辑实际入口 `appendMessageVersion(user)` 均改走 graft，assistant 路径未提前改动并留给 MSG-R2。红色回归先证明旧实现 active path 截止于新 user，绿化后 domain 与 repository 两层均得到 `U1/A1/U2(v2)/A2`，branch ID/数量不变、旧 U2 仍在；graph/projector/repository 32/32 与 analyze 通过。ADR-0001 同步修订为默认 graft 合同。
...
PD-01/PD-02 已于 2026-07-12 修订，方案与 ADR-0001 均已同步；MSG-R3/MSG-R1 已完成。Phase 2 的 MSG-04 仅剩 §10.3 MSG-R2 重生成设置分流，Phase 4 继续等待 TL-R14/R15。完成前 Phase 5 不开工。Windows/Linux 的 `DB2_CAPABILITY_RESULT` 原始行与 TL-08 非 macOS 实机交互仍留在发布门禁。

## 19. 下一步

继续按 §10.3 冻结顺序返工：(1) MSG-R2 重生成按设置分流：默认 graft 保留下方，开启删除尾随时只 fork、不物理删除；(2) TL-R14 同帧唯一滚动执行器 + 手势内不翻转意图；(3) TL-R15 jump 管线同帧输入与未布局目标兜底。之后执行用户真机四症状矩阵：发送置顶稳定、流式中点"到底"/慢速拖拽贴底无往复位移、"仅保存"与默认重生成后下方消息保留且切换器可用、开启删除尾随设置后行为符合预期。矩阵通过后恢复 MSG-04、TL-02/TL-04 与 Phase 2/4 为完成，再进入 Phase 5 `OPS-01`。

## 20. 变更日志

| 日期 | 变更 | 工作项 | Commit/PR | 作者 |
| --- | --- | --- | --- | --- |
| 2026-07-12 | 完成 MSG-R1：新增单事务 graft command，创建同 slot sibling 后把 active path 直接 child 改挂到新 revision（leaf 时移动原 branch leaf），映射 boundary 并 CAS 推进 state revision；branch ID/数量与全部后续 slot 不变，旧 revision 保留。稳定 graph edit API 与 Home 编辑实际调用的 `appendMessageVersion(user)` 均接入；红色回归复现旧 fork 截断，绿化后的 domain/repository 两层都保持完整未来。ADR-0001 同步修订；graph/projector/repository 32/32 与 analyze 通过 | MSG-R1、MSG-04、PD-01/02、MSG-01 | 本里程碑提交 | Codex |
