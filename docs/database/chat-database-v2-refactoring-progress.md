...
| Phase 4：Timeline 与 Renderer | 2 / 8 | `进行中` | TL-01/02 完成 stable cursor、双预算窗口、陈旧请求丢弃与 ChatController 主分页接线；下一步统一视觉锚点 |
...
Phase 4 已完成 2/8。下一步进入 `TL-03`：实现以 slot ID + viewport localDy 为唯一坐标的锚点协调器，把 prepend/append/crop、版本高度变化和异步布局后的 scroll correction 统一到 ≤1 logical px 契约。
