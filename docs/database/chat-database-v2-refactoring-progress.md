...
> - 当前结论：Phase 0/1/3 保持关闭。MSG-R3 已完成：版本切换器的总数/当前索引改用 coordinator slot 的 DB 权威 `versionCount` 与 active revision，sibling 明细只在切换时懒加载，相关 64/64 与 analyze 通过。"仅保存"与重生成语义仍按修订后的默认 graft 合同进入 MSG-R1/R2；无限跳动与发送弹距进入 TL-R14/R15。§10.3 全部闭环并经用户真机四症状矩阵复测前，Phase 4 与 Phase 2 语义项保持进行中，Phase 5 暂不启动
...
| MSG-04 | Edit/regenerate/select/delete/fork commands | DB2-05、MSG-03 | `进行中`（2026-07-12 因 PD-01 修订重开，见 §10.3 MSG-R1/R2） | 全部使用稳定 revision/branch ID 且单事务；默认变更走 graft（保留后代），仅删除尾随走 fork；删除不得复用 `_rewriteMessageOrder` 或全会话 compact，D3 删除更新量不得随会话总消息数线性增长 | 本里程碑提交（2026-07-11） | v5 增加 composite FK/ordinal/kind 受约束的最小权威 `message_part_rows`，保证 edit/regenerate 不产生无正文半状态。commands 原子写 revision+text part+branch+state，selection 输入 revision ID；delete 递归更新目标因果子树和 branch-parent 子树，最新 alternate/截断 prefix 修复 active head，末 revision 要求确认，无任何 legacy order 读写。200 个无关 revision 删除反例仅 1 行 revision 进入 deleted；fork 原子 remap path/slot/revision/part ID。已知非阻塞债务：受影响 branch 检测仍对每个未删除 branch 完整投影路径，成本为 `O(活跃 branch 数 × 路径长度)`，归入 OPS-06 branch/GC 优化；复杂 parts 留 GEN-05 |
...
PD-01/PD-02 已于 2026-07-12 修订（默认 graft、仅删除尾随 fork），方案 §5.1/§7.2/§8.3 同步更新，无需用户新输入。MSG-R3 已完成；MSG-01 ADR 仍需按修订语义补充 graft 命令条目，Phase 2 的 MSG-04 继续因 §10.3 MSG-R1/R2 重开，Phase 4 继续等待 TL-R14/R15。完成前 Phase 5 不开工。Windows/Linux 的 `DB2_CAPABILITY_RESULT` 原始行与 TL-08 非 macOS 实机交互仍留在发布门禁。

## 19. 下一步

继续按 §10.3 冻结顺序返工：(1) MSG-R1/R2 graft 命令与调用点分流（先写红色回归："仅保存"后下方保留、默认重生成后下方保留且 `<1/2>` 可切换）；(2) TL-R14 同帧唯一滚动执行器 + 手势内不翻转意图；(3) TL-R15 jump 管线同帧输入与未布局目标兜底。之后执行用户真机四症状矩阵：发送置顶稳定、流式中点"到底"/慢速拖拽贴底无往复位移、"仅保存"与默认重生成后下方消息保留且切换器可用、开启删除尾随设置后行为符合预期。矩阵通过后恢复 MSG-04、TL-02/TL-04 与 Phase 2/4 为完成，再进入 Phase 5 `OPS-01`。
