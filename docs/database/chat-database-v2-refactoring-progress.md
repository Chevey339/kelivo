...
| Phase 4：Timeline 与 Renderer | 3 / 8 | `进行中` | TL-01～03 完成 stable cursor、双预算与 slotId+localDy 统一锚点；下一步滚动意图状态机与新内容 UX |
...
Phase 4 已完成 3/8。下一步进入 `TL-04`：将滚动意图冻结为 followingTail/userAnchored/programmaticJump/loading 状态机，发送新轮次定位 user slot 顶部、历史阅读不被 stream 拉走，并提供带生成/未读状态的“跳到最新”胶囊。
