...
> - 当前结论：Phase 0 止血与基线闭环。逐 chunk SQLite 与 iOS MethodChannel 等待、永久 loading、非事务 merge、缺库静默空库与每次启动 path 全库扫描均已处理；iOS background update 使用 500ms 单 writer latest-wins，chunk 只入队，finish/cancel 等待 update barrier 后才发终态。正常备份为 secret-free SQLite snapshot ZIP，旧 JSON 仍只读兼容。P0-09 已冻结 seed `20260711` 的 D1～D6、49 项快速回归和当前 M4 Pro/macOS profile 起点。基线同时暴露 D4 1 MiB 流式 Markdown 仍严重超标（build+raster p95 712.627 ms、Runner RSS peak 1.56 GiB），因此 Phase 0 完成不等于列表/renderer 性能完成；该问题进入后续 TL/renderer 工作流。raw durability、硬件断电、资源故障及其他平台证据继续由 OPS-02/DB2-07/五平台门禁追踪
