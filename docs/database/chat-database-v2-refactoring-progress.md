...
> - 最后更新：2026-07-12（第二轮返工 TL-R7～R13 实现与自动化闭环；全量 1288/1288，待用户真机复测）
> - 当前结论：Phase 0～3 保持关闭。§10.2 TL-R7～R13 已按冻结顺序全部实现：发送只保留置顶意图、完整 viewport spacer 先布局后跳转、重复胶囊删除、跨窗口定位统一 `openAround`、revision 重建保持视觉 anchor、generation 生命周期按会话隔离、删除与临时会话写入统一经过 coordinator。`flutter analyze` 与全量 1288/1288 通过；在用户真机关闭发送回弹、跨窗口跳转、并发会话和删除幽灵矩阵前，Phase 4 仍保持进行中，Phase 5 暂不启动
...
| Phase 4：Timeline 与 Renderer | 6 / 8 | `进行中` | TL-R1～R6 已获第一轮真机确认；§10.2 TL-R7～R13 实现与自动化已全部闭环，全量 1288/1288，当前只待第二轮真机关闭矩阵后恢复 TL-02/TL-04 为完成 |
...
PD-01～PD-14 已全部冻结，MSG-01 ADR 已接受，Phase 2～3 保持关闭。§10.1 TL-R1～R6 经用户真机确认有效；§10.2 TL-R7～R13 的代码、聚焦回归、全量 1288/1288 与 analyze 均已闭环，当前只待用户真机关闭矩阵，完成前 Phase 5 不开工。Windows/Linux 的 `DB2_CAPABILITY_RESULT` 原始行与 TL-08 非 macOS 实机交互仍留在发布门禁。

## 19. 下一步

执行用户真机关闭矩阵：(1) 分别发送短/长 user 消息，确认置顶后至少 400ms、开始流式、终态三个时点均不回弹且正文可见；(2) 用迷你地图跳到长会话早期消息，继续向上/向下分页不回尾部；(3) 会话 A 后台生成时切到会话 B 并开始生成，A 的终态不得收掉 B 的 spacer/生成态；(4) 删除中部消息或版本后滚动、分页、切换 anchored/following，消息不复活；(5) 临时会话发送、暂停/完成、删除仍正常且不进入历史。矩阵通过后把 TL-02/TL-04 与 Phase 4 恢复为完成，再进入 Phase 5 `OPS-01`。
