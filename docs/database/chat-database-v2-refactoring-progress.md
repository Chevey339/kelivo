...
| Phase 4：Timeline 与 Renderer | 1 / 8 | `进行中` | TL-01 stable ancestry cursor/slot page 与 Timeline Coordinator 契约完成；下一步接入双预算窗口与取消语义 |
...
Phase 4 已完成 1/8。下一步进入 `TL-02`：在 Timeline Coordinator 内加入逻辑行数 + 解码字节双预算、request epoch 取消/丢弃陈旧结果和保锚裁窗，并开始替换 ChatController 的固定 360 条物理消息窗口。
