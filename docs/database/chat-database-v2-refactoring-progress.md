...
Phase 3 已完成 5/7。下一步按方案进入 `GEN-06`：启动时从 `generation_run_rows` 原子恢复所有非终态 run 为 `interrupted`，清理对应 streaming flag，并删除 active ID JSON 作为恢复真相源。D4 renderer/RSS 超标留在 Phase 4。
