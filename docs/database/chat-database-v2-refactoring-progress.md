...
| DB2-01 schema snapshots/migration | `历史成果，兼容路径已取消` | PD-18 当前门禁回归通过 | 2026-07-13 | schema 1～10 snapshot/step 仅为研发历史，不是产品输入；schema 11 安装门只接受当前版本，非当前版本原地 fail-closed |
...
| ID | 工作项 | 依赖 | 状态 | 验收摘要 | Commit/PR | 验证证据 |
| --- | --- | --- | --- | --- | --- | --- |
| DB2-01 | Schema snapshots 与逐版本 migration tests | PD-13、P0-09 | `已完成后由 PD-18 取消兼容入口` | 最终 schema 创建/校验；中间 schema 拒绝且不改写 | 本里程碑提交 + PD-18 收口提交 | schema 11 是首个候选发布合同；安装门只接受 current，1～10 与 future schema 均 fail-closed。历史 snapshot/step 不再由产品入口调用 |
