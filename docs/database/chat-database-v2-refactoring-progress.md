...
| Phase 3：Generation State Machine | 3 / 7 | `进行中` | GEN-01～03 已完成 run begin 与网络/UI/DB 三链解耦；下一步统一 terminal 收尾 |
...
Phase 3 已完成 3/7。下一步按方案进入 `GEN-04`：把 complete/fail/cancel/interrupted 的最终 message/parts/tool/usage、run terminal CAS、兼容 streaming receipt 清理收口为一个 repository command，并让所有 failure path 在 commit 后统一清 loading。D4 renderer/RSS 超标留在 Phase 4。
