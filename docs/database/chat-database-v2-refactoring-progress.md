...
> - 追踪基线：分支 `sql`，本轮实现基线 `3409546d`
> - 最后更新：2026-07-09
> - 当前结论：正常备份使用自校验、默认排除应用已知认证凭据的 SQLite snapshot + settings/assets ZIP，不再生成 `chats.json`；旧 JSON ZIP 仍只读导入，迁移页灾难备份仍使用 JSON。v2 overwrite 已改为运行期只做同卷 durable preparation，且 candidate 只复制用户选择与 bundle 能力交集中的 DB/assets；下一次启动先持有进程级 business lease，再在首次业务持久化读取前完成 unpublished cleanup、WAL 归一化、previous 冻结、DB/settings/assets operation-ahead 切换、完整验证与显式 `rollingBack` 回滚。terminal settings 通过绑定 receipt/native PID/lease token 的 cold ack 保留 active admission，同 PID 或同 token 都不能自确认；Android 自动重启使用 process mode。macOS 专用真实 Runner harness 已以 control v2 的 25 个强类型 failpoint 完成前向切换矩阵：同一 matrix run 下，每 case 使用独立 scenario/prefix/AppData/restore run 和四个真实 Runner，跨 core 分段与 full-only targeted 运行共取得 25/25 failpoint、100 个成功 Runner phase、25 次外部 SIGKILL，并通过 13/13 support tests。该证据覆盖 claim、live DB normalize final barrier、previous settings/manifest、old live→previous 与 candidate→live 的 DB/四资源根移动、四种 receipt 的 temp/final 边界及 settings remove/first-set 部分态；仍不覆盖硬件断电、`renameAndSync` 内部 raw rename/fsync 子窗口、rollback、cold-ack/archive kill、资源故障或其他平台，因此 P0-02 保持进行中
...
| Phase 0：止血与基线 | 2 / 9 | `进行中` | P0-01/P0-08 已完成；P0-02 overwrite 主链和 macOS 25/25 前向 SIGKILL 矩阵已通过，待 rollback、terminal archive、资源故障和五平台验收 |
...
| `flutter analyze` + `flutter test`（本轮 P0-02） | `已完成` | No issues found；1027/1027 tests passed | 2026-07-09 | 当前 macOS 主机全量通过；真实 SIGKILL 矩阵证据见下方独立宿主行，仍不等于硬件断电或其他平台验证 |
...
| Restore receipt/workspace protocol | `已完成` | 36 tests passed | 2026-07-09 | append-only receipt、6-record rollback chain、精确 receipt temp、candidate/receipt 双向组件绑定、共享锁、publishing/discarding/archiving claim、terminal 顶层白名单、evidence 归档和下一 run 准入；macOS 前向矩阵已对 `oldRenamed/newInstalled/verified/committed` 的 temp durable 与 final published 共 8 个边界执行真实 SIGKILL。矩阵实测发现 unpublished 仲裁会误拒“later receipt temp + 对应 final receipt”的合法 journal，现已改为存在 final receipt 时进入 published 校验；只有 later temp 而无 final receipt 时仍 fail-closed。rollback/archive receipt kill 与 Windows rename 仍待验证 |
| Restore business lease / unpublished / terminal hardening | `已完成` | 246 focused tests passed | 2026-07-09 | 非阻塞进程/跨 isolate lease、真实子进程竞争与 kill 后重获；gate 在 lease 冲突时不触碰 prepared/live；strict no-receipt staging 耐久 discard；terminal cold ack 要求 native PID 与 token 均变化，需重写时继续阻止业务；fail-closed/rolled-back/cold-restart/restart-failure UI 均覆盖。macOS 原生 settings 跨进程读回和 25 个前向 cutover 落盘点已由下方独立 harness 覆盖；cold-ack/archive kill 与四个其他平台仍待验证 |
| macOS 真实 restore process SIGKILL harness | `已完成` | 25/25 强类型 failpoint；100 个成功 Runner phase；25 次外部 SIGKILL；13/13 support tests | 2026-07-09 | control v2 提供 smoke/core/full、`--failpoint` 与 `--from`；host 每次运行生成一个 matrix run，同一 matrix run 下每 case 使用独立 scenario/preference prefix/AppData/restore run，并由四个真实 Runner 完成 setup→kill→resume→cold finalize。25 个 case 通过跨 core 分段与 full-only targeted 成功运行完成，不记作一次单命令 full run。覆盖 claim、live DB normalization final barrier、previous settings/manifest、old live→`previous.pending` 的 DB+四 roots、`previous.pending`→`previous`、四种 receipt 的 temp durable/final published、secret remove/first sorted set 部分态、candidate→live 的 DB+四 roots；宿主仍在每次 signal 前复验 executable/PID/lstart。该证据不覆盖硬件断电、`renameAndSync` 内部 raw rename/fsync 子步骤、rollback、cold-ack/archive kill 或其他平台 |
...
| kill -9/断电/磁盘满/权限/锁库故障注入 | `进行中` | macOS 已完成 25 个前向切换独立 Runner SIGKILL case；rollback、cold-ack/archive、raw rename/fsync 子窗口和资源故障尚未覆盖 | 已证明列明的高层前向 durability 边界可跨真实进程 SIGKILL 收敛；仍不能宣称硬件断电、任意子步骤、rollback/terminal 或五平台安全 |
...
| 默认 SQLite snapshot ZIP → 当前应用 | 必须 | `进行中` | 已覆盖生成后自校验、hash/size/schema/count 拒绝、normalized candidate、运行期 prepare、下次启动完整 SQLite/settings/assets commit 或 rollback；macOS 25 个前向高层 durability failpoint 均可从真实 SIGKILL 恢复，rollback/terminal/raw 子窗口、硬件断电和五平台尚未验证 |
...
| candidate→live 后首次启动 kill | 校验 new 或回 previous | 尚未提交的 v2 尾部 | `进行中` | macOS 独立 case 已覆盖 secret remove/first sorted set、candidate DB+四资源根安装，以及 `newInstalled/verified/committed` receipt temp durable/final published；每点外部 SIGKILL 后均完成新 bundle、cold ack 与冷读归档。rollback、cold-ack/archive kill、raw rename/fsync 子步骤与其他平台仍待覆盖 |
| v2 已写新消息后代码回滚 | 使用 v2-compatible rollback | 最多一个未 checkpoint stream 尾部 | `未开始` | — |
| Restore 任一阶段 kill | 破坏性切换前旧 live 不变；开始切换后 gate 收敛到完整 new 或经验证 old，绝不放行混合 | 无已放行业务写入 | `进行中` | macOS 前向切换 25/25 强类型高层 failpoint 已完成真实 SIGKILL；同进程测试另覆盖 rollback/terminal archive 逻辑中断，未知位置 fail-closed。尚不能外推到 raw rename/fsync 子步骤、真实 rollback/terminal kill、硬件断电或其他平台 |
...
| Receipt 损坏/缺失/sequence 回退 | 进入只读恢复，不按文件名猜测 | 不开放混合 bundle | `进行中` | checksum/hash-chain 损坏、缺口、超限、非法状态与链接目录继续 fail-closed；macOS 已真实 kill 覆盖四个前向 receipt 的 temp/final 边界，并修复 unpublished 仲裁误拒“later temp + final receipt”。later temp 无 final receipt 仍 fail-closed；只读恢复入口、rollback/archive receipt kill 与多 run 诊断仍未完成 |
...
| Kill/restart recovery | 未开始 | 未开始 | 进行中：25/25 前向高层 failpoint SIGKILL 通过；rollback/terminal/raw 子窗口未覆盖 | 未开始 | 未开始 |
...
1. 继续收尾 `P0-02`：在已通过的 macOS 25-case 前向 SIGKILL 矩阵上增加 terminal archive、pre-commit rollback、cold-ack 与 `renameAndSync` raw rename/fsync 子窗口；补磁盘满、只读目录、锁库，并在 Android/iOS/Windows/Linux runner 验证 durability ABI/rename。
...
| --- | --- | --- | --- | --- |
| 2026-07-09 | 将 macOS RestoreHarness 升级为 control v2 的 25 个强类型前向 failpoint，提供 smoke/core/full、`--failpoint`/`--from`；host 每次生成一个 matrix run，同一 matrix run 下每 case 独立 scenario/prefix/AppData/restore run，以四个真实 Runner 完成 setup、外部 SIGKILL、前向恢复、cold finalize。跨 core 分段与 full-only targeted 运行取得 25/25、100 个成功 Runner phase、25 次 SIGKILL，support 13/13；实测修复 unpublished 仲裁误拒 later receipt temp+final receipt，无 final receipt 的 later temp 保持 fail-closed。未宣称单次 full、硬件断电、raw rename/fsync、rollback、cold-ack/archive kill 或其他平台 | P0-02、OPS-02 | 本里程碑提交 | Codex |
