...
| Phase 4：Timeline 与 Renderer | 5 / 8 | `进行中` | TL-01～05 完成 timeline 窗口/锚点/意图与一次性 render projection；下一步增量 Markdown、字节 LRU 与图片尺寸 |
...
| TL-04 | Following/user anchored/jump 状态机 | PD-04/05、TL-03 | `已完成` | 用户阅读历史时 stream 不强制跳动；coordinator/scroll/widget/lazy 聚焦 35/35 | 本里程碑提交（2026-07-11） | `TimelineViewportMode` 冻结 followingTail/userAnchored/programmaticJump/loading 四态；滚轮/触摸离底立即退出尾随，分页 loading 恢复原意图，stream 只更新未读/生成态而不移动 anchored viewport。新发送持久化后以 stable user slot programmatic jump 到 viewport 顶部并留出可见后续空间，重开会话同样定位最后一条 user；显式到底才恢复 following。新增带生成指示、live-region/button semantics 与中英本地化的“跳到最新”胶囊 |
...
Phase 4 已完成 5/8。下一步进入 `TL-06`：让 streaming Markdown 仅解析受影响 block，以 decoded bytes 管理 cache，并为图片预留稳定尺寸且按显示尺寸 decode。
