...
| Phase 3：Generation State Machine | 1 / 7 | `进行中` | GEN-01 已建立 generation run 唯一生命周期真相与 CAS transition；发送链切换从 GEN-02 开始 |
...
Phase 3 已完成 1/7。下一步按方案进入 `GEN-02`：把 send/regeneration 的 user revision、assistant revision、branch/state 与 generation run 收口为一次领域事务；P0-03 checkpoint writer 继续作为 GEN-03 输入。D4 renderer/RSS 超标留在 Phase 4，不在 generation 阶段扩 scope。
