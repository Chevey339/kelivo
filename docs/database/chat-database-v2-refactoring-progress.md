...
> - 最后更新：2026-07-11（Phase 2：MSG-01～03 已完成）
> - 当前结论：Message Graph 已具备稳定 ID active path projector：branch leaf 沿 revision parent 投影唯一 root→target path，目标 revision 截断不会包含未来轮次；context boundary 只接受 active ancestry 上的 revision ID，并以 state revision 原子/乐观更新。revision/branch 环、同 path 重复 slot、deleted path、fork/boundary 不一致均由可复用 validator 拒绝
...
| Phase 2：Message Graph | 3 / 7 | `进行中` | ADR、v4 schema、active path/context projector 已完成；下一步实现 edit/regenerate/select/delete/fork 原子命令 |
...
Phase 2 当前 3/7。下一步执行 MSG-04：在 repository/domain command 内实现 edit user、regenerate assistant、stable-ID selection、三类 delete 与 conversation fork；每项单事务并复用 projector 验证，删除不得全量 compact。D4 renderer/RSS 超标不在 Message Graph 阶段扩 scope。
