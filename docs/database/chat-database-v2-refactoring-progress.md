...
| TL-04 | Following/user anchored/jump 状态机 | PD-04/05、TL-03 | `进行中`（实现完成，待真机确认） | 用户阅读历史时 stream 不强制跳动；coordinator/scroll/widget/lazy 聚焦回归通过 | 本里程碑提交（2026-07-11） | `TimelineViewportMode` 冻结 followingTail/userAnchored/programmaticJump/loading 四态；滚轮/触摸离底立即退出尾随，分页 loading 恢复原意图，stream 只更新未读/生成态而不移动 anchored viewport。新发送持久化后以 stable user slot programmatic jump 并建立本轮会话专属 anchor；TL-R3 已移除 streaming 尺寸变化后的 post-frame spacer 收缩，生成锚定期改为持有 jump 当帧算出的恒定 spacer，使 content extent 不再逐 chunk `+X/-X` 振荡。锚定模式与 bottom-pin 互斥，程序滚动通知不改变视口意图，内容底部判定明确扣除 spacer；仅显式跳到最新或真实到达内容底部才恢复 following。TL-R4 统一 completed/failed/cancelled/preparation-failed 的 terminal snapshot 发布，即使消息已被裁出窗口也强制关闭 coordinator generation；终态单次将 spacer 收敛到避免 offset clamp 所需的最小残量，显式 followTail 后归零。coordinator 从 loaded streaming slot 初始化 generation 状态，覆盖首 chunk 前及重开活跃会话场景；新增带生成指示、live-region/button semantics 与中英本地化的“跳到最新”胶囊 |
...
| R-10 | 测试全绿掩盖需求反例未覆盖 | 高 | 需求矩阵与现有测试 diff | 已发生实例：Phase 4 全量 1274/1274 绿但真机流式+滑动即触发 §10.1 TL-R1/R3。TL-R1～R6 已逐项补失败回归并修至全量 1279/1279；风险继续保持进行中，直到用户真机矩阵证明新增测试确实覆盖原症状 | `进行中` |
