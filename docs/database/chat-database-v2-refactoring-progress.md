...
| Phase 4：Timeline 与 Renderer | 4 / 8 | `进行中` | TL-01～04 完成 stable cursor、双预算、统一锚点与滚动意图状态机；下一步预计算 render model 与细粒度 rebuild |
...
Phase 4 已完成 4/8。下一步进入 `TL-05`：预计算 `MessageRenderModel`，移除 row builder 内的全列表扫描，并把 provider/watch 与 rebuild 限制到真正依赖的消息槽。
