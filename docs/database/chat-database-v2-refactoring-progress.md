...
> - 最后更新：2026-07-11（Phase 2：MSG-01～05 已完成）
> - 当前结论：Hive/SQLite v1 扁平历史已有确定性 graph adapter 与 v6 migration ledger。selection 同时按 ordinal/version 解释，冲突和非法值持久化 issue；truncate 边界、duplicate/orphan/streaming 均有明确 recovered/rejected 语义。legacy 只标记 visible projection/ambiguous，不宣称恢复未知真实因果
...
| Phase 2：Message Graph | 5 / 7 | `进行中` | 原生 graph 与 deterministic legacy adapter 已完成；下一步冻结真实 legacy fixture/digest 对比 |
...
Phase 2 当前 5/7。下一步执行 MSG-06：冻结真实旧 Hive/JSON/SQLite v1 fixture，经 adapter 后对比升级前可见序列、selected revision、context/prompt digest、parts 与资源引用，并覆盖 ambiguity/reject 报告。D4 renderer/RSS 超标不在 Message Graph 阶段扩 scope。
