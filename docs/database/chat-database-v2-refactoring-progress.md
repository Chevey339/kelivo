...
| Phase 4：Timeline 与 Renderer | 8 / 8 | `已完成` | stable timeline、双预算、统一锚点、滚动意图、增量/有界 renderer 与五 TargetPlatform 交互契约全部闭环；启动顺序、retained-cache 可写性与生成期 jump spacer 回归修复后全量 1274/1274 |
...
| TL-04 | Following/user anchored/jump 状态机 | PD-04/05、TL-03 | `已完成` | 用户阅读历史时 stream 不强制跳动；coordinator/scroll/widget/lazy 聚焦回归通过 | 本里程碑提交（2026-07-11） | `TimelineViewportMode` 冻结 followingTail/userAnchored/programmaticJump/loading 四态；滚轮/触摸离底立即退出尾随，分页 loading 恢复原意图，stream 只更新未读/生成态而不移动 anchored viewport。新发送持久化后以 stable user slot programmatic jump 到 viewport 顶部并留出可见后续空间；target 完成后只要 generation 仍在进行，`0.85 × viewport` 底部 spacer 就继续保留，终态后才回收，避免短回复时 max extent 收缩造成“顶上去又弹回来”。coordinator 从 loaded streaming slot 初始化 generation 状态，覆盖首 chunk 前及重开会话场景；显式到底才恢复 following。新增带生成指示、live-region/button semantics 与中英本地化的“跳到最新”胶囊 |
...
| 2026-07-11 | 修复 programmatic jump 后短回复“顶上去又弹回来”：底部 `0.85 × viewport` spacer 不再随 target 清空立即退场，而在 generation 终态前持续保留；page replace 同步从 streaming slot 推导 `isGenerating`，使首 chunk 前和重开活跃会话也遵守同一契约，并在切换 open 时清除上一会话标记。新增 coordinator page-state 与 widget 下一帧 padding 红绿回归；相关 19/19、五平台交互用例 29 项、startup、analyze 与全量 1274/1274 通过 | TL-04 follow-up | 本里程碑提交 | Codex |
