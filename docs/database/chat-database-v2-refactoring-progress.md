...
> - 最后更新：2026-07-13（**Phase 8 已完成；凭据升级回填缺陷已修复**）
...
| P0-08 | 完整设置与凭据备份 | 无 | `已完成（产品策略已修订）` | ZIP 包含 API key/password/token 等配置并可恢复；manifest 如实声明；旧 JSON/迁移灾备和历史 secret-free bundle 可读 | 本里程碑提交（2026-07-12） | 正常 snapshot 不再 sanitizer，manifest 标记 `secretsIncluded: true`；overwrite/merge 恢复来源凭据且保留无关本机键；历史 `secretsIncluded: false` 校验与读取回归保留 |
...
| OPS-07 | 完整凭据持久化与备份恢复 | P0-08、PD-11 | `已完成（含升级缺陷修复）` | Provider/代理/WebDAV/S3/TTS/搜索凭据保存在 prefs，并由正常备份完整恢复 | 本里程碑提交（2026-07-13） | SettingsProvider 全部字段直接读写 SharedPreferences，正常 v2 备份使用完整 snapshot、manifest 声明 `secretsIncluded: true`。复测发现运行过未发布 secure-storage 实验构建的设备只剩脱敏 prefs，拆除插件后 key 不可见；现增加启动前只读回收桥，恢复旧 overlay 后删除旧 entry，且用 sanitizer 对比保护后来新填的凭据。公开 v1.1.17 Hive 升级仍直接保留原 prefs |
...
| LIN-05 | 周边口径改线性：FTS 搜索默认"各组选中版本"、开关扩全部版本；统计"选中版本/全部版本"双口径；NDJSON 默认选中版本、allRevisions 全部行；Hive→v2 迁移目标改纯线性表（ledger/rollout/退役门禁不变） | LIN-04 | `已完成`（2026-07-12，本里程碑提交） | 搜索 CTE 默认只允许选中版本命中（含 conversation EXISTS 路径），显式开关可命中旧版本；统计公开模型改名 `selectedVersions/allRevisions`；portable 默认 scope 改 `selectedVersionsCompleted`，仍只读兼容旧 `activeBranchCompleted`，allRevisions 写全部行；Hive 已在 LIN-04 直接批量写线性表且 rollout ledger 保留。聚焦 9/9、全量 1272/1272、analyze 通过 |
...
| --- | --- | --- | --- | --- |
| 2026-07-13 | 修复 Hive 升级后 Provider key 全空：根因不是聊天迁移，而是未发布 secure-storage 实验先把 prefs 凭据抽成 overlay，随后按 PD-11 拆除时遗漏回填。启动现于迁移灾备和 SettingsProvider 前读取旧 Provider/search/TTS/WebDAV/S3/global proxy secure entry，回填 SharedPreferences 后清理；若用户已重新填写则保留新值。新增主 key、多 key、scalar 与新值保护回归；`flutter analyze` 无问题，全量 1279/1279 通过 | OPS-07、PD-11、Hive 升级 hotfix | 本修复提交 | Codex |
