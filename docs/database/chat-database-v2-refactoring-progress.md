...
| Phase 3：Generation State Machine | 2 / 7 | `进行中` | GEN-01/02 已建立 run 状态机并把 persistent send/regeneration begin 收口为单事务；下一步解耦网络/UI/DB 三链 |
...
| GEN-05 | Ordered message parts/provider artifacts | MSG-02、GEN-04 | `未开始` | reasoning/tool/signature 与 final 同事务一致；parts 成为唯一正文权威 | — | 当前 `message_rows` + `message_part_rows` 正文双写是有事务保护的过渡态；完成前 repository 是唯一合法写入口，验收时须移除/拒绝绕过 repository 的 `message_rows` 正文写入，避免双源分叉 |
...
Phase 3 已完成 2/7。下一步按方案进入 `GEN-03`：以 GenerationRun identity 接管请求/stream 状态，把网络 buffer、frame-paced UI publisher 与 latest-wins DB checkpoint queue 解耦；复用 P0-03 checkpoint writer 和 iOS 500ms latest-wins updater，不重复扩大真实进程证明。D4 renderer/RSS 超标留在 Phase 4。
