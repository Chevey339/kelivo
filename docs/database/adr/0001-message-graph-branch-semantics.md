...
- 复用原 assistant slot，创建新的 assistant revision；
- 新 revision 的 parent 等于目标旧 assistant revision 的 parent；
- 创建新的 native branch，parent branch 为发起操作时包含目标 revision 的 branch，fork point 为目标 revision 的 parent；
- 新 branch leaf 为新 assistant revision，并原子切为 active branch；
- 旧 branch、旧 assistant revision 和其旧后代全部保留，但不进入新 branch prompt。
...
新 prompt 只能包含 `U1 -> A1(v2)` 的真实祖先范围，不得包含旧 branch 的 `U2/A2`。

### 4. 编辑 user

编辑 user 时：

- 复用原 user slot，创建新的 user revision；
- 新 revision 的 parent 等于目标旧 user revision 的 parent；
- 创建新的 native branch 并切为 active；
- 旧 user revision 和旧后代留在旧 branch；
- 新 assistant placeholder/run 属于 `GEN-02` 的原子 begin send，不在消息图 command 中制造半个生成状态。
...
用户在 `< n/m >` 控件选择另一个 revision 时，输入必须是目标 revision ID：

- 若已有 branch 的 active path 包含该 revision，则切换到该 branch；
- 若目标是 legacy ambiguous alternate，创建以该 revision 为 leaf 的 native branch，不把因果未知的 legacy 后续接上去；
- 若 native revision 没有现成 branch head，可从其真实 ancestor path 创建轻量 branch head；
- 切换只更新 active branch/state revision，不重编号 revision，不改写正文，不移动或复制共同祖先。

“选择一个旧 revision”不等于“让旧 branch 的后续自动成为新 revision 的后续”。后续必须来自目标 revision 的真实 parent path。
...
| 重生成旧 assistant | 目标 parent + 新 assistant revision | 旧 revision 与全部旧后代 | 把旧未来轮次加入新 prompt |
| 编辑旧 user | 目标 parent + 新 user revision | 旧 user 与全部旧后代 | 原地覆盖旧正文 |
| 切换 revision | 目标 revision 的真实 ancestor path | 其它 branch | 按 ordinal/version 猜 selection |
...
正面影响：prompt 因果关系可证明；版本选择、删除和分页使用稳定 ID；旧未来不会污染新分支；UI slot identity 与 revision 数量解耦。
