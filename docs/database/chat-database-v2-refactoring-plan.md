...
- **PD-10：旧数据保留双条件。** Hive 文件、`.previous` bundle 与 v1 库保留至"迁移后 ≥3 次成功冷启动"且"≥30 天"同时满足；清理前提供一键导出诊断包；清理写入 retention receipt，不做无记录删除。
- **PD-11：v2 不引入应用层数据库加密。** 依赖平台沙箱与文件保护；认证凭据按 OPS-07 迁入平台安全存储。SQLCipher 等数据库加密列为 v2 之后的独立评估，不阻塞本次重构。
