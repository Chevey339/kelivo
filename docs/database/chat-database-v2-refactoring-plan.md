...
上述矩阵只证明命名的高层 durability 调用已经成功返回；不覆盖 macOS `fsync → F_FULLFSYNC` 内部窗口、`renameAndSync` 的 raw rename→target-parent barrier→source-parent barrier 子窗口、raw marker delete→workspace barrier、硬件断电、verified-origin 之外的所有 rollback 起点、missing/empty/unselected previous 拓扑、terminal cold-readback/repair 的显式 before/target 部分态、`rolledBack/before` terminal 六个独立 kill 点、磁盘满、只读/权限、锁库等资源故障或其他平台。单进程 durability adapter 对 raw run rename 后中断和 parent sync 失败的测试只证明这些可见拓扑可续跑，不能记作外部 SIGKILL/断电证据。legacy markerless terminal 的 archiving marker 目前仍直接 create/write canonical；若在 create/write 子窗口被杀，系统会保留 terminal evidence 并 fail-closed，但空或截断 marker 尚不能自动收敛。后续必须实现 temp→canonical 及 startup discard/inspection 仲裁并增加低层进程测试，完成前不得宣称 legacy archive 任意 kill 安全。当前实测结果见[进度文档](./chat-database-v2-refactoring-progress.md)。
