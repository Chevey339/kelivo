...
2. 下一次启动在第一次读取 SharedPreferences、初始化窗口状态、执行 Hive/SQLite 检查或打开 ChatService 前运行唯一 restore gate；未达到可判定终态时不得创建业务 provider 或打开数据库。
...
4. receipt 使用“写临时文件 → fsync → rename → fsync 目录”的原子替换，包含单调 sequence 与 checksum；candidate、manifest 和目录也完成对应 fsync。不同平台由专用 adapter 实现并验证。
5. 启动 gate 内建立 previous bundle，live rename 为 `.previous`，持久化 `old_renamed`；candidate rename 为 live，持久化 `new_installed`。
6. reopen live，运行 quick check 和业务快照验证，标记 `verified`；应用成功启动并满足保留策略后标记 `committed`。
7. 任一步被 kill 后，下一次启动根据 receipt、DB UUID、hash 和校验结果选择唯一有效副本，绝不凭文件名猜测。receipt 损坏/缺失或同时存在多组 previous/candidate 时进入只读恢复，不自动挑选。
...
- staging 始终保存完整 bundle，不按用户本次恢复选择裁掉 DB/assets；选择的组件集合单独进入 receipt。candidate manifest 必须重写为规范化后 DB 与实际 staged entries 的 hash/size，不能继续引用 ZIP 中规范化前的 DB hash。
- manifest 声明 `includeFiles: true` 时，candidate 必须显式创建 upload/images/avatars/fonts 四个根；某根没有文件表示 overwrite 后清空该根，而不是保留 live 旧文件。`includeFiles: false` 才表示不触碰 live assets。
...
- SharedPreferences 写入完成不等于已持久落盘；启动 gate 必须 reload 并验证规范化 settings fingerprint，失败时保留 receipt 并继续恢复或阻止业务启动。
...
- 上层只有在 snapshot/ZIP 重开验证完成后才能显示“备份成功”或“恢复成功”。

### 9.7 回滚边界

- **切换前失败**：继续使用旧库，无用户数据损失。
- **切换后尚未产生 v2 写入**：可根据 receipt 回退 `.previous`。
- **切换后已有 v2 新写入**：不得直接恢复旧 snapshot，否则会丢失切换后的消息。优先发布兼容 v2 schema 的代码回滚；若必须回 v1，需导出/重放增量并明确用户可见的数据损失边界。
