...
1. candidate 执行 WAL checkpoint/TRUNCATE，关闭所有连接，确保不依赖 `-wal/-shm` 才完整。
2. receipt 使用“写临时文件 → fsync → rename → fsync 目录”的原子替换，包含单调 sequence 与 checksum；candidate、manifest 和目录也完成对应 fsync。不同平台由专用 adapter 实现并验证。
3. live rename 为 `.previous`，持久化 `old_renamed`。
4. candidate rename 为 live，持久化 `new_installed`。
5. reopen live，运行 quick check 和业务快照验证，标记 `verified`。
6. 应用成功启动并满足保留策略后标记 `committed`。
...
- 数据库一致快照优先使用 SQLite Online Backup API 或经验证的 `VACUUM INTO`。
- 便携导出采用逐行 NDJSON/chunk，不构造全量 Map/List。
...
- 默认备份不包含秘密；如产品支持秘密导出，必须显式选择并使用经过认证的加密格式。
...
| 旧 `chats.json`/ZIP | 检查格式版本、manifest/hash；流式导入 | 未知/损坏格式明确拒绝 |
...
- `OPS-01`：SQLite consistent snapshot + manifest/hash。
- `OPS-02`：staging restore/merge + crash-safe swap receipt。
- `OPS-03`：portable NDJSON v2 与旧备份 adapter。
