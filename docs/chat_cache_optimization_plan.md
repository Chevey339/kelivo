...
> 状态：实施中 —— P0（措施 1-6）与 P1（措施 7/8/9/11/13；措施 10 随 C2 删表作废）已落地：空闲补全、发送增量 append、窄查询三提交、启动并行、缓存生命周期修漏；措施 12 仅保留 12.1 冷加载骨架与 12.4 侧栏骨架——12.2 空会话占位图标与 12.3 翻页顶部 loading 行按用户决定回退（保持空白无提示），其滚动偏移配套（leadingRowCount/哨兵 diff）一并还原；经全量测试（1438 个）与对抗评审；P2（措施 14-17）待实施
