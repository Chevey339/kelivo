...
| 供应商 | 登录方式 | 聊天与模型 | 额度 |
| --- | --- | --- | --- |
| ChatGPT | 桌面浏览器 PKCE；手机设备码；桌面失败后可选择设备码 | Codex Responses，固定流式请求 | ChatGPT usage 中的主、副及额外限制窗口、可用状态和已存重置次数 |
...
ChatGPT 桌面授权使用已注册的 `http://localhost:1455/auth/callback`，端口被占用时可以关闭占用程序后重试，或选择设备码登录。设备码登录需先在 ChatGPT 个人安全设置或工作区权限中启用，见 [OpenAI 授权说明](https://learn.chatgpt.com/docs/auth)。
...
Kelivo 保留自己的 `originator` / User-Agent 和界面；手机采用 OMP 也提供的设备码流程。聊天使用完整 Responses 的 HTTP 流式传输，未引入 OMP 的可选 WebSocket / Responses Lite、CLI 模型别名或自动额度重置功能。
...
7. 手机检查从浏览器切回应用后的设备码轮询，桌面检查 ChatGPT 浏览器回调；检查明暗主题、账号长名称、长邮箱和窄窗口。
