...
void main() {
  ...
  test('a throttled code execution surfaces as a failed tool card', () {
    ...
    expect(
      chunks.whereType<ServerToolStart>().single.toolName,
      'code_execution',
    );
  });

  test('a successful code execution stays out of the tool cards', () {
    final decoder = ClaudeStreamDecoder();
    decoder.accept(
      _event('content_block_start', {
        'type': 'content_block_start',
    ...
    // Still replayable: the caller chain breaks without it.
    expect(
      decoder.assistantBlocks.any(
  ...
  test('a suppressed code execution emits no input deltas', () {
    final decoder = ClaudeStreamDecoder();
    decoder.accept(
      _event('content_block_start', {
        'type': 'content_block_start',
    ...
    // Still replayed with its input, which the caller chain needs.
    expect(
      decoder.assistantBlocks.single['input'],
      containsPair('code', 'print(1)'),
    );
  });

  test('a declared code execution surfaces its output', () {
    final decoder = ClaudeStreamDecoder(codeExecutionDeclared: true);
    decoder.accept(
      _event('content_block_start', {
        'type': 'content_block_start',
        'index': 0,
